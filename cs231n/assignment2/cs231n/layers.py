...
def batchnorm_backward(dout, cache):
    ...
    dvar = np.sum(dx_hat * x_centered * -0.5 * (sample_var + eps) ** -1.5, 
            axis=0, keepdims=True)

    dmean = -np.sum(dx_hat / sigma, axis=0, keepdims=True) + \
          dvar * -2 * np.mean(x_centered, axis=0, keepdims=True)
...
def batchnorm_backward_alt(dout, cache):
    ...
    dx = (1.0 / sigma) * (dx_hat - np.mean(dx_hat, axis=0, keepdims=True) - 
      (x - sample_mean) / sigma * np.mean(dx_hat * (x - sample_mean) / sigma, 
      axis=0, keepdims=True))
...
def layernorm_backward(dout, cache):
    ...
    dx = (1.0 / std) * (dx_hat - np.mean(dx_hat, axis=1, keepdims=True) - 
        (x - sample_mean) / std * 
        np.mean(dx_hat * (x - sample_mean) / std, axis=1, keepdims=True))
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return dx, dgamma, dbeta
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    x_cols = np.zeros((C * HH * WW, N * H_out * W_out))
    for n in range(N):
        for i in range(H_out):
            for j in range(W_out):
                ...
                col_idx = n * H_out * W_out + i * W_out + j
                x_cols[:, col_idx] = x_slice.flatten()
    
    w_cols = w.reshape(F, -1)
    
    out_cols = w_cols.dot(x_cols) + b.reshape(-1, 1)
    
    out = out_cols.reshape(F, N, H_out, W_out).transpose(1, 0, 2, 3)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    cache = (x, w, b, conv_param)

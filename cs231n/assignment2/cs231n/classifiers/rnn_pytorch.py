...
class CaptioningRNN:
    ...
    def loss(self, features, captions):
        ...
        h0 = features.mm(W_proj) + b_proj
        
        # (2) Word embeddings
        embeddings = W_embed[captions_in]  # (N, T, wordvec_dim)
        
        # (3) RNN forward
        if self.cell_type == "rnn":
            h = rnn_forward(embeddings, h0, Wx, Wh, b)  # (N, T, H)
        else:  # lstm
            h = lstm_forward(embeddings, h0, Wx, Wh, b)  # (N, T, H)
        
        # (4) Scores over vocabulary - reshape to 2D for matrix multiplication
        h_flat = h.reshape(N * T, -1)  # (N*T, H)
        scores_flat = h_flat.mm(W_vocab) + b_vocab  # (N*T, V)
        scores = scores_flat.reshape(N, T, -1)  # (N, T, V)
        
        # (5) Loss with softmax and masking
        scores_flat = scores.reshape(N * T, -1)
        captions_out_flat = captions_out.reshape(N * T)
        mask_flat = mask.reshape(N * T)
        
        loss = torch.nn.functional.cross_entropy(scores_flat, captions_out_flat, reduction='none')
    ...
    def sample(self, features, max_length=30):
        ...
        for t in range(max_length):
            # Embed current word
            word_embed = W_embed[current_word]  # (N, wordvec_dim)
            ...
            if self.cell_type == "rnn":
                ...
                next_h, next_c = lstm_step_forward(word_embed, prev_h, prev_c, Wx, Wh, b)
                prev_h, prev_c = next_h, next_c
            
            # Get scores and select best word
            scores = next_h.mm(W_vocab) + b_vocab
            current_word = torch.argmax(scores, dim=1)
            
            # Store caption
            captions[:, t] = current_word
            
            # Optional: stop if all sequences generated <END>
            if self._end is not None and (current_word == self._end).all():
                break

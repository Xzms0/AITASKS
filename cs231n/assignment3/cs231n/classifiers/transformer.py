...
class CaptioningTransformer(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        memory = self.visual_projection(features)  # (N, wordvec_dim)
        memory = memory.unsqueeze(1)  # (N, 1, wordvec_dim)
        
        tgt = self.embedding(captions)  # (N, T, wordvec_dim)
        tgt = self.positional_encoding(tgt)  # (N, T, wordvec_dim)
        
        tgt_mask = torch.tril(torch.ones(T, T))  # (T, T)，True=需要mask
        
        tgt = self.transformer(tgt, memory, tgt_mask=tgt_mask)  # (N, T, wordvec_dim)
        scores = self.output(tgt)  # (N, T, vocab_size)
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.transformer(x, src_mask=None)  # (N, num_patches, embed_dim)

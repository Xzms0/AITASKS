...
class GaussianDiffusion(nn.Module):
    ...
    def p_sample(self, x_t, t: int, model_kwargs={}):
        ...
        x_tm1 = torch.where((t == 0)[:, None, None, None], mean, x_tm1)
        ##################################################################
    ...
    def p_losses(self, x_start, model_kwargs={}):
        ...
        ####################################################################
        # 1. 从 x_start 采样 x_t（加噪声）
        x_t = self.q_sample(x_start, t, noise)  # (b, *)
        
        # 2. 调用模型预测
        # 模型输入：x_t（带噪图像），t（时间步），model_kwargs（条件）
        pred = self.model(x_t, t, model_kwargs)  # (b, *)
        
        # 3. 计算加权 MSE 损失
        # loss_weight 是 (b,) 形状，需要 reshape 到与 target 相同的维度

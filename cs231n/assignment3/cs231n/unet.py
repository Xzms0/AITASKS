...
class Unet(nn.Module):
    ...
    def cfg_forward(self, x, time, model_kwargs={}):
        ...
        # Unconditional forward pass (drop the condition)
        unconditional_kwargs = model_kwargs.copy()
        unconditional_kwargs["text_emb"] = None
        uncond_output = self.forward(x, time, unconditional_kwargs)
        
        # Apply classifier-free guidance
        x = uncond_output + (cfg_scale + 1) * (cond_output - uncond_output)
        ##################################################################
    ...
    def forward(self, x, time, model_kwargs={}):
        ...
        for down_block in self.downs:
            # First ResNet block
            x = down_block[0](x, context)
            skips.append(x)
            
            # Second ResNet block
            x = down_block[1](x, context)
            skips.append(x)
        ...
        # 3. Upsampling path
        # Reverse the skips to match upsampling blocks
        skips = skips[::-1]
        skip_idx = 0
        ...
        for up_block in self.ups:
            ...
            # First ResNet block with skip connection
            skip = skips[skip_idx]
            x = torch.cat([x, skip], dim=1)
            x = up_block[1](x, context)
            skip_idx += 1
            
            # Second ResNet block with skip connection
            skip = skips[skip_idx]
            x = torch.cat([x, skip], dim=1)
            x = up_block[2](x, context)
            skip_idx += 1

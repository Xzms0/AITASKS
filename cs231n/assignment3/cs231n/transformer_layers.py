...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        Q = Q.reshape(N, S, H, head_dim)
        K = K.reshape(N, T, H, head_dim)
        V = V.reshape(N, T, H, head_dim)
...
class PatchEmbedding(nn.Module):
    ...
    def forward(self, x):
        ...
        ############################################################################
        p = H // self.patch_size
        x = x.reshape(N, C, p, self.patch_size, p, self.patch_size)
        x = x.permute(0, 2, 4, 1, 3, 5)  # (N, p, p, C, patch_size, patch_size)
        x = x.reshape(N, p * p, self.patch_dim)
        out = self.proj(x)
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
